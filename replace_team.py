...
start_marker = '<!-- ABOUT/FOUNDER SECTION -->'
end_marker = '<!-- PREMIUM TEAM SECTION -->'
...
# go back to capture leading whitespace
while start_idx > 0 and content[start_idx-1] == ' ':
    start_idx -= 1

end_section_marker = content.find(end_marker)
if end_section_marker == -1:
    print("End not found!")
    exit()

# find closing </section> after the team section
close_tag = '</section>'
end_idx = content.find(close_tag, end_section_marker)
end_idx = end_idx + len(close_tag)
...
print(f"Content being replaced starts with: {repr(content[start_idx:start_idx+50])}")
print(f"Content being replaced ends with: {repr(content[end_idx-50:end_idx])}")

new_section = """        <!-- SERVER-RACK TEAM SECTION -->
        <style>
